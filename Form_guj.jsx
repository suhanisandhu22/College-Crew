...
import { words } from "../../data/words_guj";
import { shlok } from "../../data/shlok_guj";
...
const AudioRecorder = () => {
  ...
  useEffect(() => {
    // Simulating the gujarati model loading
  ...
  const uploadBlob = async () => {
    ...
    try {
      ...
      const stringToCheck = JSON.stringify(responseData?.text)
        ...
        .split(" ")
        .join(""); // Remove all spaces between characters
      ...
      console.log(
        "Are they equal:",
        stringToCheck.toLowerCase() === expectedText.toLowerCase()
      );
  ...
  return (
    <>
      ...
      <Popup
        ...
        receivedText={
          response ? response.replace(/\s+/g, "") : "No pronunciation detected"
        }
        expectedText={categoryData[currentCardIndex].gujarati.replace(
          /\s+/g,
          ""
        )}
      />
